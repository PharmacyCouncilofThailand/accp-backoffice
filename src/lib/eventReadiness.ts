...
export function getExternalEventReadiness(input: {
  ...
  const hasThproReadyPrimary = freePrimaryTickets.some((ticket) =>
    normalizeAllowedRoles(ticket.allowedRoles).includes("thpro")
  );

  if (!hasThproReadyPrimary) {
    warnings.push({
      code: "PRIMARY_TICKET_MISSING_THPRO",
      message: "At least one free primary ticket must allow the thpro role.",
    });
  }
